...
func (r *Rar) IPasswdCorrest(source string) int {
	if r.ImplicitTopLevelFolder {
		var err error
		_, err = r.addTopLevelFolder(source, "destination")
		if err != nil {
			return 0;
		}
	}
...
func (r *Rar) IsPasswdCorrect(source string) int {
	err := r.OpenFile(source)
	if err != nil {
		return 0
	}
	defer r.Close()
	return r.FakeUnrarNext()
}
